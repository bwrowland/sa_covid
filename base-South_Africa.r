...
countries <- read.csv('South Africa/region.csv', stringsAsFactors = FALSE)["Regions"]
...
forecast <- 35# increase to get correct number of days to simulate
...
if(DEBUG) {
  fit = sampling(m,data=stan_data,iter=2,warmup=2,chains=2)
} else if (FULL) {
